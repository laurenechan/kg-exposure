...
analysis_rf <- function(feature_selection_method = 'glmnet', nDigits = 4, maxVars = 31, 
  ...
  for (pref in prefixes){
    ...
    for (outcome in label_cols){
       ...
       use_df = little_df %>% select(starts_with(c(pref, outcome)))
        ...
        mean_LR_df = data.frame()
        for (j in 1: n_internal_iters)  {      
          balanced_df = caret::downSample(use_df, use_df[[outcome]], list = FALSE)
          ...
          print(df_LR)
          if (sum(dim(mean_LR_df))==0) mean_LR_df = df_LR
          else{
            mean_LR_df[, c("logodds","logodds_min","logodds_max","odds","odds_min","odds_max","se","pvalue" )] =
              mean_LR_df[, c("logodds","logodds_min","logodds_max","odds","odds_min","odds_max","se","pvalue" )]+
              df_LR[, c("logodds","logodds_min","logodds_max","odds","odds_min","odds_max","se","pvalue" )]
          }
        }
        mean_LR_df[, c("logodds","logodds_min","logodds_max","odds","odds_min","odds_max","se","pvalue" )] =
          round(mean_LR_df[, c("logodds","logodds_min","logodds_max","odds","odds_min","odds_max","se","pvalue" )]/n_internal_iters, nDigits)
        ...
        df_LR = prepare_LR_df( glm_fit, nDigits = nDigits)
        print(df_LR)
        writexl::write_xlsx(df_LR, file.path(dir_res, paste(outcome, 'LR_results_', pref, '.xlsx', sep ='')))
    }
